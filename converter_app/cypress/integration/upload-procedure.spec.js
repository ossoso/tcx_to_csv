...
describe('Uploading data', () => {
  ...
  it('one dataframe stored', () => {
      cy.get('#activity-jsons').should((jsonDiv) => {
        const text = jsonDiv.text()
        const serializedData = JSON.parse(text)
        expect(serializedData).to.have.length(1)
      })
  });

    it.only('dropdown appears with 1 item', () => {
      const dropdown = cy.get('#loaded-dataframes')
      dropdown.click()
      dropdown.get('div.Select-menu-outer').should((menu) => {
